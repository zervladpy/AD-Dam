    ...
    public Book get(long idBook) {
        try {
            if (con != null && !con.isClosed()) {
                // Crear Statement
                try (var st = con.prepareStatement("SELECT idBook, isbn, titulo, autor, anho, disponible, cover" +
        ...
        try {
            if (con != null && !con.isClosed()) {
                ...
                try (var ps = con.prepareStatement("INSERT "
                        + "INTO Book (isbn, titulo, autor, anho, disponible, cover)"
        ...
        try {
            if (con != null && !con.isClosed()) {
                ...
                try (var ps = con.prepareStatement("UPDATE  "
                        ...
                        + "anho=?, disponible=?, cover=? WHERE idBook= ?");) {
                    // 
                    setBookValues(book, ps);
